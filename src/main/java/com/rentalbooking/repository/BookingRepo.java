...
public interface BookingRepo extends JpaRepository<Booking, Integer> {
    @Query(value="select Booking.* from Booking inner join Bike on Booking.bike_id=Bike.id where status='AVAILABLE' and date between (?1 and ?2) and city=?3", nativeQuery=true)
